...
const ModalNotifi = (props) => {
  ...
  const isThan75kh1 =
    Math.max(...arrDiemHk1) / 3 >= 7.5 && Math.max(...arrDiemHk1) / 3 < 8.5;
  const isThan75kh2 =
    Math.max(...arrDiemHk2) / 3 >= 7.5 && Math.max(...arrDiemHk2) / 3 < 8.5;
  ...
  const scholarship45 =
    dataSelect &&
    dataSelect.includes(0) &&
    ((dataSelect && dataSelect.includes(2)) ||
      (dataSelect && dataSelect.includes(3)));
  const scholarship60 =
    dataSelect &&
    dataSelect.includes(0) &&
    dataSelect &&
    dataSelect.includes(1);
  ...
  const isError =
    infoPoint.num.every((i) => i.diem < 15) &&
    infoPoint.num2.every((i) => i.diem < 15);
  const isNaN =
    infoPoint.num.some((i) => i.diem === "NaN") ||
    infoPoint.num2.some((i) => i.diem === "NaN");
  ...
  return (
    <>
      ...
      >
        <div className="relative top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] w-full sm:max-w-[80%] md:max-w-[80%] lg:max-w-[70%] xl:max-w-[50%] max-h-full">
          {/* <!-- Modal content --> */}
            ...
            <div className="p-4 md:p-5">
              {!isError && !isNaN && !isLiet && (
                <>
                  <p className="text-base leading-relaxed text-gray-500 text-justify">
                    Chúc mừng bạn có cơ hội trúng tuyển vào chuyên ngành{" "}
                    <span className="font-bold">{infoPoint?.chuyenNganh}</span>.
                  </p>
                  <p className="text-base leading-relaxed text-justify text-gray-500 pb-3">Số điểm tổ hợp học bạ lớp 12 của bạn là:</p>
                </>
              )}
              {isError && isLiet && (
                <>
                  <p className="text-base leading-relaxed text-justify text-gray-500">
                    Rất tiếc bạn không đủ điều kiện trúng tuyển vào chuyên ngành{" "}
                    <span className="font-bold">{infoPoint?.chuyenNganh}</span>.
                  </p>
                  <p className="text-base leading-relaxed text-justify text-gray-500 pb-3">Số điểm tổ hợp học bạ lớp 12 của bạn là:</p>
                </>
              ...
              {isNaN && (
                <p className="text-lg font-semibold leading-relaxed text-red-400 pb-3">
                  Định dạng phần nhập điểm phải là số (vd: 6.8, 9,...). Vui lòng
                  bạn kiểm tra lại!
                </p>
              )}
              <p className="py-3">Học kỳ 1:</p>
              <div className="flex gap-y-2 flex-wrap mb-3 justify-between">
                {infoPoint &&
                  infoPoint?.num.map((i) => (
                    <p
                      key={i.tohop}
                      ...
                      !scholarship60 && (
                        <div className="flex items-center gap-2">
                          <p className="text-[18px]">
                            Bạn có cơ hội giành học bổng trị giá{" "}
              ...
              <div className="flex items-center justify-center px-4 pt-4 mt-3 border-t border-gray-200 rounded-b">
                {!isError && !isNaN && !isLiet && (
                  <button
                    onClick={() => setIsModalInfoPersonal(true)}
                    className="text-white bg-[#0083C2] hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
                  >
                    Đăng ký
                  </button>
                )}
                <button
                  onClick={() => setIsModalNotifi(false)}
                  className="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-red-500 focus:z-10 focus:ring-4 focus:ring-gray-100"
                >
                  Đóng
                </button>
              </div>
